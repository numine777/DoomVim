...
M.config = function()
  dvim.builtin["terminal"] = {
    ...
    -- { exec, keymap, name}
    -- dvim.builtin.terminal.execs = {{}} to overwrite
    -- dvim.builtin.terminal.execs[#dvim.builtin.terminal.execs+1] = {"gdb", "tg", "GNU Debugger"}
    execs = {
      -- { "lazygit", "gg", "LazyGit" },
    },
  }
...
M.setup = function()
  ...
  for _, exec in pairs(dvim.builtin.terminal.execs) do
    require("dvim.core.terminal").add_exec(exec[1], exec[2], exec[3])
  end
  terminal.setup(dvim.builtin.terminal)

  if dvim.builtin.terminal.on_config_done then
    dvim.builtin.terminal.on_config_done(terminal)
  end
end
...
M.add_exec = function(exec, keymap, name)
  vim.api.nvim_set_keymap(
    "n",
    "<leader>" .. keymap,
    "<cmd>lua require('dvim.core.terminal')._exec_toggle('" .. exec .. "')<CR>",
    { noremap = true, silent = true }
  )
  dvim.builtin.which_key.mappings[keymap] = name
end
