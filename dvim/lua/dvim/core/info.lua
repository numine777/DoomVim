...
local lsp_utils = require "dvim.lsp.utils"
...
local function get_formatter_suggestion_msg(ft)
  local null_formatters = require "dvim.lsp.null-ls.formatters"
  ...
  if not vim.tbl_isempty(supported_formatters) then
    vim.list_extend(section, {
      ...
      "",
      fmt("  dvim.lang.%s.formatters = { { exe = '%s' } }", ft, table.concat(supported_formatters, "│")),
    })
  end
...
local function get_linter_suggestion_msg(ft)
  local null_linters = require "dvim.lsp.null-ls.linters"
  ...
  if not vim.tbl_isempty(supported_linters) then
    vim.list_extend(section, {
      ...
      "",
      fmt("  dvim.lang.%s.linters = { { exe = '%s' } }", ft, table.concat(supported_linters, "│")),
    })
  end
...
function M.toggle_popup(ft)
  ...
  local function set_syntax_hl()
    vim.cmd [[highlight dvimInfoIdentifier gui=bold]]
    vim.cmd [[highlight link dvimInfoHeader Type]]
    vim.cmd [[let m=matchadd("dvimInfoHeader", "Language Server Protocol (LSP) info")]]
    vim.cmd [[let m=matchadd("dvimInfoHeader", "Formatters and linters")]]
    vim.cmd('let m=matchadd("dvimInfoIdentifier", " ' .. ft .. '$")')
    vim.cmd 'let m=matchadd("string", "true")'
    vim.cmd 'let m=matchadd("error", "false")'
    tbl_set_highlight(registered_providers, "dvimInfoIdentifier")
    -- tbl_set_highlight(require("dvim.lsp.null-ls.formatters").list_available(ft), "dvimInfoIdentifier")
    -- tbl_set_highlight(require("dvim.lsp.null-ls.linters").list_available(ft), "dvimInfoIdentifier")
  end

  local Popup = require("dvim.interface.popup"):new {
