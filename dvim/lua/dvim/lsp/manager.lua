...
local function is_overridden(server)
  local overrides = dvim.lsp.override
...
local function resolve_config(name, user_config)
  local config = {
    on_attach = require("dvim.lsp").common_on_attach,
    on_init = require("dvim.lsp").common_on_init,
    capabilities = require("dvim.lsp").common_capabilities(),
  }

  local status_ok, custom_config = pcall(require, "dvim.lsp/providers/" .. name)
  if status_ok then
...
function M.setup(server_name, user_config)
  ...
  local function ensure_installed(server)
    ...
    if not dvim.lsp.automatic_servers_installation then
