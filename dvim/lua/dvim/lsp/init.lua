...
local function lsp_highlight_document(client)
  if dvim.lsp.document_highlight == false then
...
local function lsp_code_lens_refresh(client)
  if dvim.lsp.code_lens_refresh == false then
    return
  end
...
local function add_lsp_buffer_keybindings(bufnr)
  ...
  if dvim.builtin.which_key.active then
    ...
    for mode_name, mode_char in pairs(mappings) do
      wk.register(dvim.lsp.buffer_mappings[mode_name], { mode = mode_char, buffer = bufnr })
    end
  else
...
local function select_default_formater(client)
  ...
  for _, filetype in ipairs(client_filetypes) do
    if dvim.lang[filetype] and #vim.tbl_keys(dvim.lang[filetype].formatters) > 0 then
...
function M.setup()
  ...
  require("dvim.utils").toggle_autoformat()
end
