...
function M.setup()
  local config = { -- your config
    virtual_text = dvim.lsp.diagnostics.virtual_text,
    signs = dvim.lsp.diagnostics.signs,
    underline = dvim.lsp.diagnostics.underline,
    update_in_insert = dvim.lsp.diagnostics.update_in_insert,
    severity_sort = dvim.lsp.diagnostics.severity_sort,
  }
  if vim.fn.has "nvim-0.5.1" > 0 then
    vim.lsp.handlers["textDocument/publishDiagnostics"] = function(_, result, ctx, _)
      ...
      if ok then
        ...
        for i, sign in ipairs(dvim.lsp.diagnostics.signs.values) do
